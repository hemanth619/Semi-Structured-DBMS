...
public class BTLeafPage extends BTSortedPage {
	 ...
	 */
	public BTLeafPage(PageId pageno, int keyType) throws IOException, ConstructPageException {
	 ...
	 * @exception ConstructPageException BTLeafPage constructor error
	 */
	public BTLeafPage(Page page, int keyType) throws IOException, ConstructPageException {
		super(page, keyType);
		setType(NodeType.LEAF);
	}
	...
	public KeyDataEntry getFirst(RID rid) throws IteratorException {
		...
		try {
			...
			entry = BT.getEntryFromBytes(getpage(), getSlotOffset(0), getSlotLength(0), keyType, NodeType.LEAF);
	...
	public KeyDataEntry getNext(RID rid) throws IteratorException {
		...
		try {
			...
			entry = BT.getEntryFromBytes(getpage(), getSlotOffset(i), getSlotLength(i), keyType, NodeType.LEAF);
	 ...
	 */
	boolean redistribute(BTLeafPage leafPage, BTIndexPage parentIndexPage, int direction, KeyClass deletedKey)
