...
public class IndexUtils {
			...
			ConstructPageException
	{
		...
		if (selects[2] == null) {
			...
			if (selects[0].op.attrOperator == AttrOperator.aopEQ) {
				if (selects[0].type1.attrType != AttrType.attrSymbol) {
					key = (IntervalKey) getValue(selects[0], selects[0].type1, 1);
					indScan = ((IntervalTreeFile)indFile).new_scan(key, key);
				}
				else {
					key = (IntervalKey) getValue(selects[0], selects[0].type2, 2);
					indScan = ((IntervalTreeFile)indFile).new_scan(key, key);
				}
				return indScan;
			}
			...
			// aopAD
			if (selects[0].op.attrOperator == AttrOperator.aopAD || selects[0].op.attrOperator == AttrOperator.aopPC  || selects[0].op.attrOperator == AttrOperator.aopCP) {
				if (selects[0].type1.attrType != AttrType.attrSymbol) {
					
					key = (IntervalKey)getValue(selects[0], selects[0].type1, 1);
					
					IntervalKey loKey = new IntervalKey(null);
					IntervalKey hiKey = new IntervalKey(null);
					
					IntervalType loType = new IntervalType(key.getKey().start, 0 , 0);
					IntervalType hiType = new IntervalType(key.getKey().end, 0 , 0);
					
					loKey.setKey(loType);
					hiKey.setKey(hiType);
					indScan = ((IntervalTreeFile)indFile).new_scan(loKey, hiKey);
				}
				else {
					key = (IntervalKey)getValue(selects[0], selects[0].type2, 2);
					
					//IntervalType dummyInt = new IntervalType(0,0,0);
					IntervalKey loKey = new IntervalKey(new IntervalType(0,0,0));
					IntervalKey hiKey = new IntervalKey(new IntervalType(0,0,0));
