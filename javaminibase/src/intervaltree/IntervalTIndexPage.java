...
public class IntervalTIndexPage extends IntervalTSortedPage {
	...
	/*
	 * find a key by B++ algorithm, but returned key may not equal the key passed
	 * in.
	 * 
	 ...
	 *
	 */
	KeyClass findKey(KeyClass key) throws IndexSearchException {
	...
	KeyDataEntry findKeyData(KeyClass key) throws IndexSearchException {
		...
		try {
			...
			for (int i = getSlotCnt() - 1; i >= 0; i--) {
				entry = IntervalT.getEntryFromBytes(getpage(), getSlotOffset(i), getSlotLength(i), keyType,
						NodeType.INDEX);
	...
	/**
	 * Left Link You will recall that the index pages have a left-most pointer that
	 * is followed whenever the search key value is less than the least key value in
	 * the index node. The previous page pointer is used to implement the left link.
	 * 
	 ...
	 * followed whenever the search key value is less than the least key value in
	 * the index node. The previous page pointer is used to implement the left link.
	...
	int getSibling(KeyClass key, PageId pageNo) throws IndexFullDeleteException {
		...
		try {
			...
			for (i = getSlotCnt() - 1; i >= 0; i--) {
				...
				if (BT.keyCompare(key, entry.key) >= 0) {
					if (i != 0) {
						entry = IntervalT.getEntryFromBytes(getpage(), getSlotOffset(i - 1), getSlotLength(i - 1),
			...
			entry = IntervalT.getEntryFromBytes(getpage(), getSlotOffset(0), getSlotLength(0), keyType,
					NodeType.INDEX);
			pageNo.pid = ((IndexData) entry.data).getData().pid;
	 ...
	 * 
	 * @param direction -1 if "this" is left sibling of indexPage ; 1 if "this" is
	 ...
	 */
	boolean redistribute(IntervalTIndexPage indexPage, IntervalTIndexPage parentIndexPage, int direction,
			KeyClass deletedKey) throws RedistributeException {
