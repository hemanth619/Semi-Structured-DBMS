...
import btree.BT;
import intervaltree.IntervalTIndexPage;
import intervaltree.IntervalTSortedPage;
...
public class IntervalTLeafPage extends IntervalTSortedPage {

	/**
	 * pin the page with pageno, and get the corresponding BTLeafPage, also it sets
	 * the type to be NodeType.LEAF.
	 * 
	 * @param pageno  Input parameter. To specify which page number the BTLeafPage
	 *                will correspond to.
	 ...
	 */
	public IntervalTLeafPage(PageId pageno, int keyType) throws IOException, ConstructPageException {
		super(pageno, keyType);
		setType(NodeType.LEAF);
	}

	/**
	 * associate the BTLeafPage instance with the Page instance, also it sets the
	 * type to be NodeType.LEAF.
	 * 
	 * @param page    input parameter. To specify which page the BTLeafPage will
	 *                correspond to.
	 * @param keyType either AttrType.attrInteger or AttrType.attrString. Input
	 *                parameter.
	 * @exception IOException            error from the lower layer
	 * @exception ConstructPageException BTLeafPage constructor error
	 */
	public IntervalTLeafPage(Page page, int keyType) throws IOException, ConstructPageException {
		super(page, keyType);
		setType(NodeType.LEAF);
	}

	/**
	 * new a page, associate the BTLeafPage instance with the Page instance, also it
	 ...
	 * @exception ConstructPageException BTLeafPage constructor error
	 */
	public IntervalTLeafPage(int keyType) throws IOException, ConstructPageException {
		super(keyType);
		setType(NodeType.LEAF);
	}

	/**
	 * insertRecord READ THIS DESCRIPTION CAREFULLY. THERE ARE TWO RIDs WHICH MEAN
	 * TWO DIFFERENT THINGS. Inserts a key, rid value into the leaf node. This is
	 * accomplished by a call to SortedPage::insertRecord() Parameters:
	 * 
	 ...
	 */
	public RID insertRecord(KeyClass key, RID dataRid) throws LeafInsertRecException {
		KeyDataEntry entry;
	 ...
	 * 
	 * @param rid It will be modified and the first rid in the leaf page will be
	...
	public KeyDataEntry getFirst(RID rid) throws IteratorException {
		...
		try {
			...
			entry = IntervalT.getEntryFromBytes(getpage(), getSlotOffset(0), getSlotLength(0), keyType, NodeType.LEAF);
	 ...
	 * iterator interface to the records on a BTLeafPage.
	 * 
	 * @param rid It will be modified and the next rid will be passed out by itself.
	...
	public KeyDataEntry getNext(RID rid) throws IteratorException {
		...
		try {
			...
			entry = IntervalT.getEntryFromBytes(getpage(), getSlotOffset(i), getSlotLength(i), keyType, NodeType.LEAF);
	...
	/**
	 * getCurrent returns the current record in the iteration; it is like getNext
	 * except it does not advance the iterator.
	 * 
	 ...
	 * 
	 * @param direction -1 if "this" is left sibling of leafPage ; 1 if "this" is
		...
		try {
			if (direction == -1) { // 'this' is the left sibling of leafPage
				...
				if ((getSlotLength(0) + available_space() + 8) > ((MAX_SPACE - DPFIXED) / 2)) {
					...
					KeyDataEntry firstEntry;
					firstEntry = IntervalT.getEntryFromBytes(getpage(), getSlotOffset(0), getSlotLength(0), keyType,
							NodeType.LEAF);
